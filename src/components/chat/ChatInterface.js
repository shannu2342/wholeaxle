...
const ChatInterface = ({ route, navigation }) => {
    ...
    if (currentConversation) {
      const unreadMessageIds = (currentConversation.messages || [])
        .filter(
          (msg) =>
            !msg.read &&
            String(msg.senderId || msg.sender) !==
              String(user?.id || user?._id),
        )
        .map((msg) => msg.id);

      dispatch(fetchMessages({ chatId: currentConversation.id }));
  ...
  const renderMessage = ({ item }) => (
    <ChatBubble
      message={item}
      isOwn={
        String(item.senderId || item.sender) === String(user?.id || user?._id)
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <ChatFooter activeFilter="Chats" onFilterChange={handleFilterChange} />
      ...
      <View style={styles.chatContainer}>
        <FlatList
          ...
          keyExtractor={(item, index) =>
            item.id || item._id || `${item.timestamp || "msg"}-${index}`
          }
