...
const generateMockMessage = (chatId, senderId, content, type = 'text', hoursAgo = 0) => ({
    id: generateMockId(),
    chatId,
    senderId,
    content,
...
const generateMockConversation = (vendorId, vendorName, lastMessageContent, hoursAgo = 0) => ({
    id: generateMockId(),
    vendorId,
    vendorName,
    vendorAvatar: Math.random() > 0.5 ? `https://via.placeholder.com/48?text=${vendorName.charAt(0)}` : null,
    ...
    timestamp: generateMockTimestamp(hoursAgo),
    messages: [
        generateMockMessage(generateMockId(), vendorId, `Hello! Welcome to ${vendorName}`, 'text', 24),
        generateMockMessage(generateMockId(), 'user', 'Hi there! I saw your products online.', 'text', 23),
        generateMockMessage(generateMockId(), vendorId, lastMessageContent, 'text', hoursAgo),
    ],
});
...
class MockChatService {
            ...
            on: (event, callback) => {
                ...
                if (event === 'offer_received') {
                    setTimeout(() => {
                        callback({
                            id: generateMockId(),
                            vendorName: vendorData[Math.floor(Math.random() * vendorData.length)].name,
                            offerData: {
    ...
    async getConversations(userId, filter = null) {
        console.log(`🔧 Mock ChatService: Fetching conversations for user: ${userId}, filter: ${filter}`);
        ...
        console.log(`🔧 Mock ChatService: Returning ${filtered.length} conversations`);
        return filtered;
    }
    ...
    async sendMessage(messageData) {
        ...
        const conversation = this.conversations.find(conv => conv.id === messageData.chatId);
        if (conversation) {
    ...
    async markAsRead(chatId, messageIds) {
        console.log(`🔧 Mock ChatService: Marking messages as read`, { chatId, messageIds });
    ...
    async searchMessages(chatId, query) {
        console.log(`🔧 Mock ChatService: Searching messages in ${chatId} for "${query}"`);
    ...
    getConversationStats() {
        ...
        const unreadConversations = this.conversations.filter(c => c.unreadCount > 0).length;
        const totalUnreadMessages = this.conversations.reduce((sum, c) => sum + c.unreadCount, 0);
        const conversationsWithOffers = this.conversations.filter(c => c.hasOffers).length;
...
export const enableMockMode = () => {
    ...
    chatService.getConversations = mockChatService.getConversations.bind(mockChatService);
    ...
    chatService.searchMessages = mockChatService.searchMessages.bind(mockChatService);
